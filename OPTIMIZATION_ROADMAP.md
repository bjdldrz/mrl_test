...
### 智能体显式星间转发动作 v2(2026-07-07)

**本次修正**:v1 中星间转发是环境在源星无法直接下传时自动触发的规则式 fallback,智能体无法决定是否转发、何时转发、转给谁。v2 将星间转发升级为策略动作。

**动作语义**:
1. 启用 `--enable_inter_satellite_transfer` 后,多星动作空间新增 `n_satellites - 1` 个转发动作槽。
2. 对每颗源星而言,每个转发动作对应一个目标卫星。
3. 若智能体选择某个目标星,环境会把源星当前星上仍占用存储的全部未交付图片一次性发送给目标星。
4. 星间转发暂不受星间可见窗口约束,只消耗固定 `--inter_satellite_transfer_time_s`。
5. 动作 mask 只在以下条件满足时开放:源星当前有可转发图片、目标星有足够存储容量、且所有图片都能通过目标星在 deadline/horizon 前下传到基站。
6. 转发动作执行后,源星时间推进到 `current_time + inter_satellite_transfer_time_s`,源星存储在转发结束时释放,目标星承担后续下传存储占用。

**实现要点**:
- `envs/multi_satellite_env.py`
  - full action: `任务槽位 + 转发目标槽位 + idle`;
  - candidate/v2 action: `候选任务槽位 + 转发目标槽位 + idle`;
  - 转发动作不参与任务冲突拍卖,由选择该动作的源星单独执行。
- `algo/mappo_trainer.py`
  - 意图广播和联合探索只对真实任务动作做去重,不再把转发动作误判为任务冲突。
- `cva_mappo_v2/run_experiment.py`
  - raw valid action 诊断改用多星环境完整 mask,能统计转发动作。

**后续可扩展**:
1. 加入卫星-卫星可见窗口和链路速率。
2. 允许智能体选择“部分发送”或按任务价值排序发送。
3. 为接收卫星加入接收确认/拒收动作,形成显式协商机制。
