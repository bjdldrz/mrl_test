...
| 学习式任务分配器 | 🟡 分步实现中(2026-06-23) | `learned_assignment_v1`: heuristic/MLP/LSTM/GRU/Transformer/Set Transformer scorer |
...
4. **L3 图匹配分配器(下一步/研究级)**:把卫星-任务可见关系建成二分图,用 GNN 或 cross-attention 输出边权,再用 Hungarian/auction/greedy 解码,保持硬约束不被神经网络破坏。
