...
本文档默认采用最新版联合约束口径:卫星完成观测后,图像需要在可见基站窗口内下传才算任务完成,并受单星星上存储容量约束。若需要复现实验早期“观测即完成”的旧口径,删除 `--n_ground_stations/--downlink_time_s/--satellite_storage_capacity` 相关参数,或显式设为 `--n_ground_stations 0 --downlink_time_s 0 --satellite_storage_capacity 0`。
...
- `--scenario_cache_dir` 启用预生成场景池;训练读取 `train_scenarios.pkl`,评估只读取 `eval_scenarios.pkl`,避免 train/eval 泄漏。
- 使用 `--scenario_cache_dir` 时,实际评估 episode 数以缓存中的 `eval_scenarios.pkl` 为准;`--eval_episodes` 只作为运行记录/命名参考。
- `--candidate_action_top_k` 控制多星低层策略的候选动作空间;0 为 full action,128 是当前压力场景推荐值。
- `--no_viz` 会跳过可视化 JSON,避免并行评估完成后为了可视化额外串行重跑 1 个 episode。
- `cva_mappo_v2.run_experiment` 当前默认 deterministic eval;若要随机采样评估,显式添加 `--eval_stochastic`。
- 当 `--device cuda:0` 且希望评估并行时,必须显式添加 `--eval_device cpu`;否则底层会按 GPU 评估处理,单卡场景自动降为 1 个 eval worker。
- 总 CPU 压力大约是 `jobs × (train_env_workers + eval_workers)`;如果机器是 16 核,批量消融建议从 `--jobs 2 --train_env_workers 4 --eval_workers 4` 开始。

## 2. 预生成场景与 VTW

正式压力实验建议先生成固定场景池。训练集按简单到复杂 curriculum 组织,评估集使用独立 seed 生成,两者不相同。
...
快速调试时可以把 `--n_train_scenarios` 改为 20,或加 `--no_warm_vtw` 只保存任务场景。
...
```bash
--jobs 2 --train_env_workers 4 --eval_episodes 16 --eval_workers 4 --eval_device cpu
```
...
```bash
--jobs 2 --train_env_workers 4 --eval_workers 4
```
