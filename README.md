...
当前论文主方案 CVA-MAPPO 的训练、对比和消融执行流程见 [CVA_MAPPO_EXPERIMENT_PLAN.md](CVA_MAPPO_EXPERIMENT_PLAN.md)。
...
- **吞吐调优**:`train.py` 支持 `--num_workers`、`--meta_batch_size`、`--inner_steps`、`--rollout_steps`、`--eval_interval`、`--eval_workers`、`--vtw_time_step_s`。`train_log.csv` 会记录 `sample_s/modulation_s/worker_map_s/meta_opt_s/eval_s`,优先用这些列定位瓶颈,不要用 GPU 利用率判断 MRL-DMS 是否正常。
...
只对比集合/注意力式 Transformer/Set Transformer 分配 scorer:
...
训练型消融会自动按训练池和评估规模扩容 `max_action_dim`,避免动态任务槽位不足污染完成率。`--fast` 下评估间隔为 5;`--meta_iterations 2` 只适合接口 smoke,若要看 `best_eval_reward` 至少设为 6。`--n_routine/--n_dynamic` 在 `meta_encoder_v1` 中表示 eval 任务规模;如果日志提示“未指定 ACLED 数据”,请先确认 `./DynamicMission/DynamicMission.shp` 文件存在且路径有效。

MRL-DMS 低 GPU 利用率时,优先调整 CPU 并行和评估频率。吞吐诊断命令:
