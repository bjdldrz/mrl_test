...
- **吞吐调优**:`train.py` 支持 `--num_workers`、`--meta_batch_size`、`--inner_steps`、`--rollout_steps`、`--eval_interval`、`--vtw_time_step_s`。`train_log.csv` 会记录 `sample_s/modulation_s/worker_map_s/meta_opt_s/eval_s`,优先用这些列定位瓶颈,不要用 GPU 利用率判断 MRL-DMS 是否正常。
